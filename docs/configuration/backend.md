...
- `cache_dir`: Where the preprocessed data will be written on disk, defaults to the location of the input dataset. See [Remote Filesystem](../user_guide/remote_filesystems.md#remote-dataset-cache) for more details
- `cache_credentials`: Optional dictionary of credentials (or path to credential JSON file) used to write to the cache. See [Remote Filesystem](../user_guide/remote_filesystems.md#using-different-cache-and-dataset-filesystems) for more details
