...
- `max_concurrent_trials`: The maximum number of trials to train concurrently. Defaults to `auto` if not specified.
  - `auto` will infer the max_concurrent_trials that can be set given your cluster configuration if you're using a Ray backend.
  - `max_concurrent_trials` will be set to None if using a Local or Horovod backend.
